...
public class Ripemd160 {
	...
	public Ripemd160(byte[] digest) {
		...
		if(digest != null){
			mDigestBytes = new byte[digest.length];
			System.arraycopy(digest, 0, mDigestBytes, 0, digest.length);
		}
		else{
			mDigestBytes = null;
		}
	...
	public byte[] bytes() {
		...
		byte [] digest = new byte[mDigestBytes.length];
		System.arraycopy(mDigestBytes, 0, digest, 0, mDigestBytes.length);
//		return mDigestBytes;
		return digest;
	}
