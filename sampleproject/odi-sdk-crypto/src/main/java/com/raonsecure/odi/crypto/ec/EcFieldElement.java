...
public class EcFieldElement {
	...
	public EcFieldElement divide(EcFieldElement b) {
		return new EcFieldElement(_q,
				_x.multiply(b.toBigInteger().modInverse(_q)).mod(_q));
	}
	...
	public EcFieldElement sqrt() {
		...
		if (_q.testBit(1)) {
			// z = g^(u+1) + p, p = 4u + 3
			EcFieldElement z = new EcFieldElement(_q,
					_x.modPow(_q.shiftRight(2).add(BigInteger.ONE), _q));
		...
		do {
			...
			while (P.compareTo(_q) >= 0 || !(P.multiply(P).subtract(fourQ)
					.modPow(legendreExponent, _q).equals(qMinusOne)));
	...
	private static BigInteger[] lucasSequence(BigInteger p, BigInteger P, BigInteger Q,
			BigInteger k) {
		int n = k.bitLength();
